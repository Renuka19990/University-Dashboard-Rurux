...
const registerStudent = async (req, res) => {
    ...
    try {
        ...
        // Get the highest StudentId and increment it
        const lastStudent = await StudentModel.findOne().sort({ StudentId: -1 }).exec();
        const StudentId = lastStudent ? lastStudent.StudentId + 1 : 1;

        console.log(`Last StudentId: ${lastStudent ? lastStudent.StudentId : 'None'}, New StudentId: ${StudentId}`);

        // Check if StudentId is a valid number
        if (isNaN(StudentId)) {
            throw new Error("StudentId is not a valid number");
        }
...
const loginStudent = async (req, res) => {
    ...
    try {
        ...
        if (!Student) {
            return res.status(401).send("Student not found. Try to register first.");
        }
        bcrypt.compare(password, Student.password, (err, result) => {
            if (err) throw new Error(err.message);
            if (result) {
